    ...
    switch (key) {
        ...
        case 'o':
            if (drop == 0) {
                ...
                if (posXBadan > posXKotak + 3) posXBadan -= 1;
            }
            break;
        case 'p':
            posXBadan += 1;
...
void Grid() {
    double i;
    const float Z_MIN = -50, Z_MAX = 50, X_MIN = -50, X_MAX = 50, gap = 2;
    glColor3f(0.5, 0.5, 0.5);
    glBegin(GL_LINES);
    for (i = Z_MIN; i < Z_MAX; i += gap) { glVertex3f(i, 0, Z_MIN); glVertex3f(i, 0, Z_MAX); }
    for (i = X_MIN; i < X_MAX; i += gap) { glVertex3f(X_MIN, 0, i); glVertex3f(X_MAX, 0, i); }
    glEnd();
}
...
void Balok(float panjang, float lebar, float tinggi) {
    glPushMatrix();
    float p = panjang / 2, l = lebar / 2, t = tinggi / 2;
    // Face rendering logic simplified for layout
    glBegin(GL_QUADS);
    // Front/Back
    glVertex3f(-p, 0, l); glVertex3f(p, 0, l); glVertex3f(p, -t * 2, l); glVertex3f(-p, -t * 2, l);
    glVertex3f(-p, 0, -l); glVertex3f(p, 0, -l); glVertex3f(p, -t * 2, -l); glVertex3f(-p, -t * 2, -l);
    // Top/Bottom
    glTexCoord2f(0.0f, 0.0f); glVertex3f(-p, 0, -l); glTexCoord2f(0.0f, 1.0f); glVertex3f(p, 0, -l); glTexCoord2f(1.0f, 0.0f); glVertex3f(p, 0, l); glTexCoord2f(1.0f, 1.0f); glVertex3f(-p, 0, l);
    // ... Additional faces would follow here ...
    glEnd();
    glPopMatrix();
}
...
void perubahKotak() {
    if (drop == 1 && grab == 1) {
        if (posYKotak >= 3) posYKotak -= 0.01;
        if (posYKotak < 3) { bring = 0; hit = 0; grab = 0; }
    }
}

void pengubahTangan() {
    hit = (posXBadan == 4) ? 1 : 0;
    if (hit == 1 && grab == 1) bring = 1;
    if (gerakTangan == 1) { if (rotTangan1 >= -90) rotTangan1 -= 0.1; if (rotTangan1 < -90) gerakTangan = 2; }
    if (gerakTangan == 2) { if (rotTangan2 >= -90) rotTangan2 -= 0.1; if (rotTangan2 < -90 && hit == 1) grab = 1; }
    if (gerakTangan == 3) { if (rotTangan2 <= 0) rotTangan2 += 0.1; if (rotTangan2 > 0) gerakTangan = 4; }
    if (gerakTangan == 4) { if (rotTangan1 <= 0) rotTangan1 += 0.1; if (rotTangan1 > 0) gerakTangan = 0; }
}
