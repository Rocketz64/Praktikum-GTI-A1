...
void Balok(float panjang, float lebar, float tinggi)
{
    ...
    glVertex3f(-p, 0, -l); glVertex3f(p, 0, -l); glVertex3f(p, -t * 2, -l); glVertex3f(-p, -t * 2, -l);
    glEnd();
    // Simplified rendering for brevity; logic maintained
    glPopMatrix();
}

void pergerakanKaki()
{
    if (kick == 1) { if (rotKaki <= 45) rotKaki += 0.03; if (rotKaki > 44.9) kick = 2; }
    if (posXBola > -2.9) touch = 1; else if (posXBola < -12) touch = 0;
    if (kick == 2) { if (rotKaki >= -90) { rotKaki -= 0.2; if (rotKaki < 1 && touch == 1) roll = 1; } if (rotKaki < -90) kick = 3; }
    if (kick == 3) { if (rotKaki <= 0) rotKaki += 0.05; if (rotKaki > -1) kick = 0; }
}

void pergerakanBola()
{
    if (roll == 1) { if (jarak > 0) { posXBola -= 0.03; jarak -= 0.01; } if (jarak < 0) { roll = 0; jarak = 1; } }
}
